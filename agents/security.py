...
class SecurityAgent:
    ...
    def analyze_directory(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future_to_file = {
                executor.submit(self._analyze_file, file_info['path'], bandit_issues): file_info
...
"""
        ...
        try:
            ...
            response_content = self._get_llm_response(messages)
            
            # Parse response
            findings = self._parse_llm_response(response_content, str(file_path))
    ...
    def _get_llm_response(self, messages: List) -> str:
        ...
        try:
            ...
            # Check if it's a rate limit or actual failure vs fallback
            error_msg = str(e)
            ...
            short_msg = error_msg.split('\n')[0][:200]
            logger.warning(f"⚠️ Primary LLM failed in Security Agent, falling back to Ollama: {short_msg}")
